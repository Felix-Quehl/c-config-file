...
int read_config_file(char *config_path, struct Configuration *configuration)
{
    ...
    if (filePointer != NULL)
    {
        ...
        while (!feof(filePointer))
        {
            ...
            if (!strcmp(label, HARD_CODE_STRING(configuration->Just)))
                strcpy(configuration->Just, value);

            else if (!strcmp(label, HARD_CODE_STRING(configuration->Some)))
                strcpy(configuration->Some, value);

            else if (!strcmp(label, HARD_CODE_STRING(configuration->Values)))
                strcpy(configuration->Values, value);
        }
        fclose(filePointer);
    }
    else
...
void print_config(struct Configuration *configuration)
{
    ...
    printf(HARD_CODE_STRING(configuration->Just));
    printf(" = %s\n", configuration->Just);
    printf(HARD_CODE_STRING(configuration->Some));
    printf(" = %s\n", configuration->Some);
    printf(HARD_CODE_STRING(configuration->Values));
